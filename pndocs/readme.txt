$Id$

ContactList
===========
http://code.zikula.org/contactlist


content follows :-) (perhaps)

You can use some alternative tempaltes. Please take a look in the 
tempaltes directory and replace the regular file with the 
alternative file of a block etc. - if there is such a file


module developers take a look at developers.txt please!


Modules that integrate ContactList
==================================
- ContactList:	1.0	ingored users cannot send buddy-requests
- EZComments:	1.6	ignored users cannot comment a user's content
- TimeIt:		1.1	Calendar sharing options "events for friends only"
					Ignored user's calendar entries are not shown
- pnMessages:	2.0 SVN	Ignored users cannot send private messages any mre